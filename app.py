...
def generate_quiz_page(router):
    ...
    st.session_state['difficulty_level'] = st.selectbox("Select difficulty level:", ["Easy", "Medium", "Hard"], key="difficulty_select")
...
def quiz_page(router):
    if st.session_state['questions']:
        ...
        for i, question_item in enumerate(st.session_state['questions']):
            ...
            st.write(f"**{question}**")
            st.session_state['user_answers'][i] = st.radio(f"Select your answer for question {i+1}", options, key=f"q_{i}")
        ...
        if st.session_state.get('show_answers'):
            ...
            if 'questions' in st.session_state and 'correct_answers' in st.session_state:
                for i, question_item in enumerate(st.session_state['questions']):
                    st.write(f"**Question {i+1}:** {question_item['question']}")
